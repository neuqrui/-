    ...
    def train(self, replay_buffer, num_iter):
        ...
        for epoch in range(self.args.epochs):
            ...
            while batch_idx < int(data_num / self.args.batch_size):
                ...
                # record loss
                pi_losses.update(l_pi.item(),  self.args.batch_size)
                ...
                batch_idx += 1
                # plot progress
                bar.suffix = '({batch}/{size}) Data: {data:.2f}s | Batch: {bt:.2f}s | All: {total:} | ETA: {eta:} | L_pi: {lpi:.3f} | L_v: {lv:.3f} | L_reg: {lreg:.3f}'.format(
                        batch=batch_idx,
