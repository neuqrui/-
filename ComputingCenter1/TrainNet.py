...
class TrainNet:
            ...
            while batch_idx < int(data_num / self.args.batch_size):
                ...
                if self.args.cuda:
                    ...
                    for i in range(self.args.num_net):
                        new_data, target_policy[i], target_values[i], iter_num[i] = new_data.contiguous().cuda(), target_policy[i].contiguous().cuda(), target_values[i].contiguous().cuda(), iter_num[i].contiguous().cuda()
                data_time.update(time.time() - start)
                # gpu parallel train
                out_pi, out_v = self.net_work_parallel(data) if self.args.cuda and self.args.gpu_parallel else self.net_work(new_data)
    ...
    def calculate_step_loss(self, replay_buffer):
        ...
        for layers, _, net_pi, pi, net_v, v in replay_buffer.trajectory:
            step_loss_p[layers] = (step_loss_p[layers] * num_layers[layers] + self.loss_pi(torch.tensor(pi, dtype=torch.float64), torch.tensor(net_pi, dtype=torch.float64), torch.tensor([1], dtype=torch.float64)).numpy().tolist() * self.args.board_size ** 2) / (num_layers[layers] + 1)
