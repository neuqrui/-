...
class ChessBoard(object):
    ...
    def add_file_title(self, list, title):  # 创建一个新的txt文件,并写入己方和对方的名称
            f = open("棋谱_" + title + ".txt", "w+", encoding="utf-8")
            stri = ''
            for i in list:
                stri = stri+i
            f.write(stri+';')
    ...
    def save_data(self, r, chess, title):  # 将数据写入文件
        f = open("棋谱_" + title + ".txt", "a+", encoding="utf-8")
        f.write('\n' + str(r) + ' ')
        if len(chess) == 1:
            f.write(chess[0])
        elif len(chess) == 2:
    ...
    def delete_data(self, list, title):  # 悔棋
        f = open("棋谱.txt", "w", encoding="utf-8")
        self.add_file_title(list, title)
        f.write("")
        f.close()
